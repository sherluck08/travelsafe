...
class compound(object):
    ...
    def enabled_for_config(self, config):
        return not any(predicate(config) for predicate in self.skips.union(self.fails))
...
class SpecPredicate(Predicate):
    ...
    def __call__(self, config):
        ...
        dialect, driver = self.db.split("+") if "+" in self.db else (self.db, None)
...
class LambdaPredicate(Predicate):
    def __init__(self, lambda_, description=None, args=None, kw=None):
        ...
        self.lambda_ = (lambda db: lambda_()) if not spec[0] else lambda_
        self.args = args or ()
        self.kw = kw or {}
        if description:
            self.description = description
...
class OrPredicate(Predicate):
    ...
    def __call__(self, config):
        return any(pred(config) for pred in self.predicates)
